...
CREATE TABLE dbo.payment_methods (
    ...
    last_four CHAR(4) NULL,  -- Only last 4 digits, never stored raw
    expiry_month TINYINT NULL,
    expiry_year SMALLINT NULL,
    billing_name_encrypted VARBINARY(MAX) NULL,  -- AES-256 encrypted
    billing_address_encrypted VARBINARY(MAX) NULL,  -- AES-256 encrypted
    billing_zip_encrypted VARBINARY(MAX) NULL,  -- AES-256 encrypted
    token NVARCHAR(255) NOT NULL,  -- Payment processor token (not raw card data)
    token_fingerprint NVARCHAR(64) NOT NULL,  -- Hash of token for deduplication
    ...
    CONSTRAINT UQ_payment_methods_token_fingerprint UNIQUE (token_fingerprint, customer_id),
...
CREATE TABLE dbo.payments (
    ...
    payment_processor_id NVARCHAR(255) NULL,  -- Stripe, Square, etc charge ID
    receipt_url_encrypted VARBINARY(MAX) NULL,  -- Encrypted URL to receipt
    processed_at DATETIMEOFFSET(0) NULL,  -- Null until successfully processed
    idempotency_key NVARCHAR(64) NOT NULL,  -- Prevent duplicate charges
...
CREATE INDEX idx_payments_transaction_reference ON dbo.payments(transaction_reference);
GO
...
CREATE TABLE dbo.audit_logs (
    ...
    resource_type NVARCHAR(100) NOT NULL,  -- 'payment', 'payment_method', 'billing', etc
    resource_id NVARCHAR(255) NOT NULL,  -- ID of the resource affected
    action NVARCHAR(50) NOT NULL,  -- 'create', 'update', 'delete', 'view'
    event_data NVARCHAR(MAX) NOT NULL CONSTRAINT DF_audit_logs_event_data DEFAULT '{}',  -- Redacted/hashed sensitive data
...
CREATE INDEX idx_audit_logs_customer_id ON dbo.audit_logs(customer_id);
CREATE INDEX idx_audit_logs_event_type ON dbo.audit_logs(event_type);
CREATE INDEX idx_audit_logs_resource_type ON dbo.audit_logs(resource_type);
